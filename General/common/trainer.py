...
def epoch_loop(config, model, loss_functions, optimizer, scheduler, train_loader, val_loader, epoch_idx):
    stop_training = False
    ...
    model.train()
    total_train_loss = 0
    train_iterator = tqdm(train_loader, desc=f"Epoch {epoch_idx+1}/{config.epochs} [Train]", leave=False)
    ...
    try:
        for i, (batch_X, batch_Y) in enumerate(train_iterator):
            ...
            for name, weight, fn in loss_functions:
                if weight > 0:
                    loss_val = fn(predictions, batch_Y, internals)
        ...
        try:
            with torch.no_grad():
                for batch_X_val, batch_Y_val in val_iterator:
                    ...
                    val_loss = 0
                    for _, weight, fn in loss_functions:
                        if weight > 0:
...
def train_model(config, model, loss_functions, train_loader, val_loader):
    ...
    if config.load_checkpoint and os.path.exists(config.ckpt_path):
        ...
        config.start_epoch = ckpt.get('start_epoch', 0)
    ...
    try:
        for epoch_idx in range(config.start_epoch, config.epochs):
            _, avg_val_loss, stop_training = epoch_loop(config, model, loss_functions, optimizer, scheduler, train_loader, val_loader, epoch_idx)
            if stop_training:
                break
