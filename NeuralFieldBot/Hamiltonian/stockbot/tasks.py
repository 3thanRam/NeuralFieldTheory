# stockbot/tasks.py - Orchestrates stockbot training and testing.
import torch
import os
import numpy as np
...
def run_testing(args):
    ...
    # We need the bias for testing too, to build the model correctly
    ...
    predictions = _autoregressive_predict(model, initial_sequence, pred_len, config.device)
    ...
    _plot_candlestick_comparison(
        ...
        "test_results/stock_comparison_plot.png"
    )
...
def _plot_candlestick_comparison(history, ground_truth, predictions, save_path):
    ...
    def draw_candles(data, start_index, color_up, color_down):
        for i, ohlcv in enumerate(data):
            ...
            o, h, l, c = ohlcv[0], ohlcv[1], ohlcv[2], ohlcv[4]
